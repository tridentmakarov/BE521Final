function [output] = Step4PostprocessSVD(dg_train, sp, fingerFeats, f_i, binary)
...
binary = [binary; 0];
binary_class = zoInterp(binary, 50);
...
peaks_max = [];
peaks_max = max(sp(binary_class==1));
...
if ~isempty(peaks_max) % Checks to make sure peaks exist
	...
	sp(binary_class) = sp(binary_class) * peak_ratio;
...
a = 1;
sp(~binary_class) = filtfilt(b1,a,sp(binary_class));
